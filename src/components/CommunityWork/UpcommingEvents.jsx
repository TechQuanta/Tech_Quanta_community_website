...
// --- Utility Function: Parse DD-MM-YYYY Date and Extract Time from H:MM or ISO String ---
/**
 * Parses a date string (DD-MM-YYYY) and a time string (H:MM or ISO 8601)
 * to create a JavaScript Date object.
 * It prioritizes the date from dateString and extracts only the time components
 * from timeString, handling both H:MM and ISO 8601 formats.
 *
 * @param {string} dateString - Date in 'DD-MM-YYYY' format.
 * @param {string} timeString - Time in 'H:MM' or 'YYYY-MM-DDTHH:mm:ss.sssZ' format.
...
const parseDateTime = (dateString, timeString) => {
    ...
    const month = parseInt(dateParts[1], 10) - 1; // Month is 0-indexed (0-11)
    ...
    if (timeString) {
        // Check if it's an ISO 8601 string (contains 'T' and 'Z' for UTC)
        if (timeString.includes('T') && timeString.includes('Z')) {
            ...
            if (!isNaN(isoTimeObj.getTime())) {
                ...
                // Using UTC helps avoid local timezone issues if the API sends UTC times.
    ...
    // Construct the final Date object using the date parts (year, month, day)
    // and the extracted time parts (hours, minutes).
...
// --- Digit Component for Countdown (Unchanged) ---
const Digit = ({ value }) => {
...
// --- Countdown Timer Component (Unchanged, relies on parseDateTime) ---
const CountdownTimer = ({ eventDateString, eventTimeString }) => {
    ...
    useEffect(() => {
        ...
        const calculateTimeLeft = () => {
            ...
            if (difference <= 0) {
                setTimeLeft(null); // Stop countdown when time is up
                return;
            }
    ...
    // Render logic for the timer
    if (targetTimeMs === null) {
        return null; // Don't render if date is invalid or not provided
    }

    if (!timeLeft) {
        // If timeLeft is null, it means event has started or is invalid
        ...
        return null; // Or potentially a "Loading/No upcoming event" message
    }
...
export default function SliderShowcase() {
    ...
    useEffect(() => {
        const fetchSlides = async () => {
            ...
            try {
                ...
                // The map function already provides a fallback image, so filtering
                // based on 'image' presence is unnecessary and could discard slides.
                const formattedSlides = fetchedEvents.map(event => ({
                    ...
                    time: event.event_time || '', // Allow empty string for parseDateTime to handle
                }));

                if (formattedSlides.length === 0) {
                    console.warn("API returned no events or all events had invalid data after formatting.");
                    setHasError(true); // Treat as an error if no slides are valid
                    setSlides([]);
                } else {
                ...
                console.error('Failed to fetch event slides from API or process data:', error);
                ...
                if (cachedSlides) {
                    try {
                        ...
                        if (parsedCachedSlides.length > 0) {
                            ...
                            // If cached slides are empty, still show error
                            setHasError(true);
                            setSlides([]);
                            console.error('Cached slides were empty. Showing error.');
                        }
                    } catch (parseError) {
                    ...
                    setHasError(true);
                    setSlides([]);
                    console.error('No cached slides available. Showing error.');
                }
            } finally {
                setIsLoading(false);
            }
    ...
    // Memoized callback for smooth slide transitions
    const transitionToSlide = useCallback((newIndex) => {
        if (isAnimating || slides.length === 0) return;

        setIsAnimating(true);
        setIsFadeIn(false); // Start fade-out

        setTimeout(() => {
            setCurrentIndex(newIndex);
            setIsFadeIn(true); // Start fade-in for new slide
    ...
    useEffect(() => {
        ...
        setIsFadeIn(true); // Initial fade-in when slides are loaded
        clearTimeout(autoAdvanceTimeoutRef.current);
        autoAdvanceTimeoutRef.current = setTimeout(showNextSlide, SLIDE_DELAY);

        return () => clearTimeout(autoAdvanceTimeoutRef.current); // Clear on unmount or re-render
    ...
    return (
        ...
        >
            ...
            {/* Custom Styles for Animations and Effects (Unchanged) */}
            <style>{`
