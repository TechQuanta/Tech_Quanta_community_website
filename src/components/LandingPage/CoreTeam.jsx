...
import TeamDetails from "../ui/teamdetails";

const items = TeamDetails;
...
const ProfileDetailBlock = ({ member, isLoading, theme }) => {
  ...
  const cardBgClass = isDark
    ? "bg-gray-900/40 border-indigo-500/50"
    : "bg-white/40 border-indigo-400/50";
    ...
    return (
      <div
        ...
        className={`relative flex items-center justify-center p-8 backdrop-blur-lg rounded-2xl transition-all duration-500 
                    ${
                      isDark
                        ? "bg-gray-900/30 border border-indigo-700/50"
                        : "bg-white/30 border border-indigo-400/50"
                    }`}
      ...
      >
        <div className="flex flex-col items-center">
          <FaCodeBranch
            ...
            } text-6xl mb-4 opacity-70 animate-pulse-slow`}
          />
          <p
            className={`text-xl font-semibold text-center ${secondaryTextColor}`}
          >
            Click on a team member to load their profile.
          </p>
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`absolute inset-0 border-4 border-indigo-500 rounded-2xl transition-opacity duration-1000 animate-pulse-slow 
        ...
        {isLoading && (
          <div
            className={`absolute inset-0 flex flex-col items-center justify-center rounded-2xl z-10 opacity-100 transition-opacity duration-300 ${loadingBgClass}`}
          >
        ...
        <div className={`flex flex-col items-center w-full ${appearanceClass}`}>
          <img
            ...
            loading="lazy"
          />
          <h3 className={`text-3xl font-extrabold ${primaryTextColor}`}>
            {member.name}
          </h3>

          {/* Role added back per user request */}
          <p className={`text-xl font-medium mb-3 ${accentColor}`}>
            {member.role}
          </p>

          {/* The description is the detailed field intended for the profile view */}
          <p
            className={`text-sm text-center mb-6 max-w-xs italic border-t border-indigo-500/30 pt-4 ${secondaryTextColor}`}
          >
            {member.description}
          </p>
...
const CoreTeam = () => {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        // Modified: Removed 'obs' argument and unobserve() call
  ...
  const CARD_BASE_CLASSES =
    "p-3 sm:p-4 backdrop-blur-md rounded-xl transition-all duration-300 cursor-pointer relative overflow-hidden";

  return (
    // Main container with transparent background
    <div
      className={`py-20  bg-transparent ${primaryTextColor} min-h-screen transition-colors duration-500`}
    >
      ...
      <div className="max-w-7xl">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          >
            ...
            <div
              className={`absolute left-0 top-0 w-1 bg-teal-500  transition-all duration-[3000ms] ease-out 
            ...
            <div className="pl-6 space-y-4">
              {/* Title Block */}
              <div
                className={`flex flex-col space-y-2 pb-2 border-b border-indigo-500/30 ${
                  isDark ? "" : "border-indigo-700/30"
                }`}
              >
                <div
                  className={`flex items-center space-x-3 ${accentIconColor}`}
                >
              ...
              {/* Text Content */}
              <p
                className={`leading-relaxed text-lg pt-4 ${secondaryTextColor}`}
              >
                The TechQuanta community thrives on the{" "}
                <strong>unwavering dedication</strong> of our core team. These
                individuals are the{" "}
                <strong>architects, mentors, and innovators</strong> who
                volunteer their time and expertise to foster a supportive and
                productive open-source environment.
              </p>
              <p
                className={`leading-relaxed text-md ${lightSecondaryTextColor}`}
              >
                Every feature, every piece of documentation, and every
                encouraging word is a testament to their commitment. They embody
                the spirit of collaboration that defines us.
                <strong>Click on a profile</strong> on the right to learn more
                about the contributors shaping our future!
              </p>
